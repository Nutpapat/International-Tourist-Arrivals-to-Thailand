"""Number_of_foreigner_travel_to_thailand_in_summer_season"""
import pandas as pd
import pygal
def main():
    """7 land mass foreigner travel to thailand between 2013 to 2018 in summer_season"""
    data = pd.read_csv('data.csv')
    lis_y = []
    lis_x = [2013, 2014, 2015, 2016, 2017, 2018]
    check = 0
    group = data.groupby(['Season'])
    dataframe = group.get_group('summer')
    ...
    chart.set_title ='ปริมาณชาวต่างชาติที่เข้ามาเที่ยวประเทศไทยตอนฤดูร้อนช่วง 6 ปี'
    chart.set_xlabel = 'ปี'
    ...
    chart.render_to_file('GraphSummerSeason.svg')
